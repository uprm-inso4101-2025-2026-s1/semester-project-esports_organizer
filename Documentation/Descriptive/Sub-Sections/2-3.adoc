...
[.hl-yellow]#The implementation stage translates the requirements described in Section 2.2 into a working system. While requirements define *what* the platform must achieve, the implementation details *how* these goals were realized.#

[.hl-green]#For Milestone 2, this section has been expanded to include current progress on the Profiles module, showing concrete examples of backend and frontend integration that transform previous design concepts into a functional system.#
...
* [.hl-green]#A backend service (Node.js/Express) now handles the Profiles module, which interacts with Firestore and the React client through REST endpoints.#
* [.hl-green]#The local environment has been successfully tested and launched, confirming connectivity between modules and databases.#

[.hl-green]#The `/profiles` endpoint was used for testing profile retrieval functionality in the local development environment.#
...
* [.hl-green]#**Profiles module (Milestone 2 addition):** Integrates CRUD operations and privacy settings.#

[.hl-green]#Implemented functions such as `getParticipatedTournaments()` within the Profiles module retrieve user data directly from Firestore, ensuring up-to-date synchronization between the backend and the database.#

[.hl-green]#Additionally, asynchronous functions like `saveProfile`, `getProfile`, and `updateProfile` manage the creation, retrieval, and updating of user profiles within Firestore, maintaining accurate and timestamped records.#

[.hl-green]#**Architecture Diagram:**#
[.hl-green]#The following UML/Architecture diagram illustrates how the Profiles module connects the frontend React client with the backend API, Firebase Authentication, Firestore, and related modules.#
...
creation.
* [.hl-green]#A UML or architecture diagram now illustrates how the Profiles module interacts with the Badges and PrivacyControls components (see `profiles_architecture.svg`).#
* Figma mockups illustrate how the interface supports usage scenarios.

=== 2.3.1 Selected Fragments of Implementation

[.hl-yellow]#Selected fragments are only included when they clarify explanations and complement the documentation.#

[.hl-red]#For this stage of the project, no fragments are presented since the system is still under development. Once code and designs are available, they will be added to illustrate how Section 2.2 requirements are met in practice.#

[.hl-green]#For Milestone 2, the following code excerpts from the Profiles module demonstrate Firestore integration and CRUD functionality. These fragments show how user-related data is retrieved, created, and updated through asynchronous operations, confirming full connectivity between the backend and the Firestore database.#
...
[.hl-green]#The following fragment complements the previous example by showing how user profile information is created, retrieved, and updated. This code implements the CRUD logic for the Profiles module and ensures that user data is stored with proper timestamps and validation.#
